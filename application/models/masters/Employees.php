...
class Employees extends CI_Model
{
	...
	public function ssp_table()
	{
		...
		$data['columns'] = array(
			...
			array('db' => 'employee_id', 'dt' => 3, 'field' => 'employee_id'),
	...
	{
		$this->load->model(['base_model', 'setting/code_generators', 'masters/users']);
		$file_img = $_FILES['image'];
		if (!empty($file_img['name'])) {
			$path = 'assets/uploads/img/employees/';
		...
		$employee_id = empty($post_data['employee_id']) ? $this->code_generators->generate_code('employees') : $post_data['employee_id'];
