                                ...
                                <div class="box-body">
                                    <div class="box-group" id="accordion">
                                        ...
                                        foreach ($checkups as $checkup) {
                                            ...
                                            $data['is_in'] = $no == 1 ? 'in' : '' ;
                                            $data['id'] = $checkup->id;
                                            $data['medical_record_id'] = $checkup->medical_record_id;
                                            $data['poly_name'] = $checkup->poly_name;
                                            $data['doctor_name'] = $checkup->doctor_name;
                                            $data['date_in'] = $checkup->date_in;
                                            $data['complaint'] = $checkup->complaint;
                                            if (!empty($checkup->diagnose_id)) {
                                                $data['diagnose_id'] = $checkup->diagnose_id;
                                                $data['diagnose_date'] = $checkup->diagnose_date;
                                                $data['description'] = $checkup->description;
                                                $data['total_price'] = $checkup->total_price;
                                                $data['flag'] = $checkup->flag;
                                                $data['diagnose_drugs'] = $diagnose_drugs;
                                                $data['diagnosedrug_sales'] = $diagnosedrug_sales;
                                            }
                                            $this->load->view('pages/' . $class_link . '/patients/detail_medical_histories', $data);
                                        }
                                        /* print_r($checkups);
                                        print_r($diagnose_drugs);
                                        print_r($diagnosedrug_sales); */
                                        ?>
                                    </div>
                                </div>
                            </div>
