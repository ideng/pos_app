...
class Custom_ssp {
    ...
    static function order ( $request, $columns, $isJoin = false )
    {
        ...
        if ( isset($request['order']) && count($request['order']) ) {
            $orderBy = array();
            $dtColumns = Custom_ssp::pluck( $columns, 'dt' );
            ...
            for ( $i=0, $ien=count($request['order']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['orderable'] == 'true' ) {
                    ...
                    /* My Custom */
                    $my_columns = strpos($column['db'], ' AS ') !== FALSE ? explode(' AS ', $column['db']) : $column['db'] ;
                    $my_columns = is_array($my_columns) ? $my_columns[0] : $my_columns ;

                    $orderBy[] = ($isJoin) ? $my_columns.' '.$dir : '`'.$my_columns.'` '.$dir;
                    /* My Custom */
                }
            }
    ...
    static function filter ( $request, $columns, &$bindings, $isJoin = false )
    {
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    $binding = Custom_ssp::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
                    // $globalSearch[] = ($isJoin) ? $column['db']." LIKE ".$binding : "`".$column['db']."` LIKE ".$binding;

                    /* My Custom */
                    $my_columns = strpos($column['db'], ' AS ') !== FALSE ? explode(' AS ', $column['db']) : $column['db'] ;
                    $my_columns = is_array($my_columns) ? $my_columns[0] : $my_columns ;
                    $globalSearch[] = ($isJoin) ? $my_columns." LIKE ".$binding : "`".$my_columns."` LIKE ".$binding;
                    /* My Custom */
                }
            }
        }
        ...
        for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
            ...
            if ( $requestColumn['searchable'] == 'true' &&
                ...
                $my_columns = strpos($column['db'], ' AS ') !== FALSE ? explode(' AS ', $column['db']) : $column['db'] ;
                $my_columns = is_array($my_columns) ? $my_columns[0] : $my_columns ;
                $columnSearch[] = ($isJoin) ? $my_columns." LIKE ".$binding : "`".$my_columns."` LIKE ".$binding;
                /* My Custom */
            }
        }
    ...
    {
        $bindings = array();
        $db = Custom_ssp::sql_connect( $sql_details );

        // Build the SQL query string from the request
        $limit = Custom_ssp::limit( $request, $columns );
        $order = Custom_ssp::order( $request, $columns, $joinQuery );
        $where = Custom_ssp::filter( $request, $columns, $bindings, $joinQuery );
        ...
        if($joinQuery){
            ...
            $col = Custom_ssp::pluck($columns, 'db', $joinQuery);
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS ".implode(", ", Custom_ssp::pluck($columns, 'db'))."
			 FROM $table
			 $where
			 $extraWhere
			 $groupBy
        ...
        $data = Custom_ssp::sql_exec( $db, $bindings, $query);

        // Data set length after filtering
        $resFilterLength = Custom_ssp::sql_exec( $db,
            "SELECT FOUND_ROWS()"
        );
        ...
        $resTotalLength = Custom_ssp::sql_exec( $db,$count_request);
        ...
        return array(
            ...
            "data"            => Custom_ssp::data_output( $columns, $data, $joinQuery )
        );
    }
    ...
    static function sql_exec ( $db, $bindings, $sql=null )
    {
        ...
        try {
            ...
            Custom_ssp::fatal( "An SQL error occurred: ".$e->getMessage() . " - " . $sql );
        }
